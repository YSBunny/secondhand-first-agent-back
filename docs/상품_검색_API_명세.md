...
> **이 PR은 문서만 추가한다. 코드 변경이 없다.**
> 읽고 시작하기 위한 것이라 머지 여부와 무관하게 브랜치에서 바로 봐도 된다.
...
`Product.deliveryFee`(값 객체)에 담긴다. `ProductUpsertService`가 AI 응답의
`delivery_fee`를 옮기므로 **적재할 때 따로 할 일은 없다.**

```java
@Embedded
private DeliveryFee deliveryFee;   // minFee · homeFee · payer
```

| 필드 | 뜻 |
| --- | --- |
| `minFee` | 편의점 반값택배를 **포함한** 최저가 |
| `homeFee` | 편의점 픽업이 **필요 없는** 수단 중 최저가 |

두 값을 나눈 이유가 있다. 편의점 택배가 가장 싸지만 **주변에 그 편의점이 없는
사용자에게는 선택지가 아니다.** 최저가만으로 비교하면 그런 사용자에게 실제보다 싸 보인다.

**`homeFee`가 `null`이면 편의점 픽업 외에 방법이 없다는 뜻이다.** 0으로 채우면
그 매물이 "무료배송"으로 보여 총액 1위로 올라간다.

> 실측 140건 중 12건에서 두 값이 달랐고, 38건은 집앞 배송이 불가했다.
